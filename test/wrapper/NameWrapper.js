...
describe('Name Wrapper', () => {
  ...
  async function registerSetupAndWrapName(label, account, fuses, expiry = 0) {
    ...
    await NameWrapper.wrapARB2LD(label, account, fuses, expiry, EMPTY_ADDRESS)
  }
  ...
  describe('unwrap()', () => {
    ...
    it('Will not unwrap .arb 2LDs.', async () => {
      ...
      await NameWrapper.wrapARB2LD(label, account, 0, 0, EMPTY_ADDRESS)
      const ownerOfWrappedARB = await NameWrapper.ownerOf(
        namehash('unwrapped.arb'),
      )
      expect(ownerOfWrappedARB).to.equal(account)
      await expect(
  ...
  describe('wrapARB2LD()', () => {
    ...
    it('Can re-wrap a name that was wrapped has already expired on the .arb registrar', async () => {
      ...
      const tx = await NameWrapper2.wrapARB2LD(
        label,
        account2,
        CAN_DO_EVERYTHING,
        0,
      ...
      // Check the 4 events
      // UnwrapARB2LD of the original owner
      // TransferSingle burn of the original token
      // WrapARB2LD to the new owner with fuses
    ...
    it('emits Wrap event', async () => {
      await BaseRegistrar.register(labelHash, account, 84600)
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
      const tx = await NameWrapper.wrapARB2LD(
        label,
        account,
        CAN_DO_EVERYTHING,
        0,
    ...
    it('Allows an account approved by the owner on the .arb registrar to wrap a name.', async () => {
      ...
      await NameWrapper2.wrapARB2LD(label, account, 0, 0, EMPTY_ADDRESS)
    ...
    it('Does not allow anyone else to wrap a name even if the owner has authorised the wrapper with the ENS registry.', async () => {
      ...
      await expect(
        NameWrapper2.wrapARB2LD(label, account, 0, 0, EMPTY_ADDRESS),
    ...
    it('Does not allow the controller of a name to wrap it if they are not also the registrant.', async () => {
      ...
      await expect(
        NameWrapper2.wrapARB2LD(label, account2, 0, 0, EMPTY_ADDRESS),
      ).to.be.reverted
    })
    ...
    it('Will not wrap an empty name', async () => {
      ...
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
      await expect(
        NameWrapper.wrapARB2LD('', account, CAN_DO_EVERYTHING, 0, ZERO_ADDRESS),
    ...
    it('Rewrapping a previously wrapped unexpired name retains PCC and expiry', async () => {
      ...
      // unwrap it
      await NameWrapper.unwrapARB2LD(labelHash, account, account)
  ...
  describe('unwrapARB2LD()', () => {
    ...
    it('emits Unwrap event', async () => {
      ...
      const tx = await NameWrapper.unwrapARB2LD(labelHash, account, account)
      await expect(tx)
        .to.emit(NameWrapper, 'NameUnwrapped')
    ...
    it('Does not allows an account authorised by the owner on the ENS registrar to unwrap a name', async () => {
      ...
      await expect(
        NameWrapper2.unwrapARB2LD(labelHash, account2, account2),
      ).to.be.revertedWith(`Unauthorised("${nameHash}", "${account2}")`)
    })
    ...
    it('Does not allow a name to be unwrapped if CANNOT_UNWRAP fuse has been burned', async () => {
      ...
      await expect(
        NameWrapper.unwrapARB2LD(labelHash, account, account),
      ).to.be.revertedWith(
  ...
  describe('upgradeARB2LD()', () => {
    ...
    it('Upgrades a .arb name if sender is owner', async () => {
      ...
      //set the upgradeContract of the NameWrapper contract
      await NameWrapper.setUpgradeContract(NameWrapperUpgraded.address)
      const tx = await NameWrapper.upgradeARB2LD(label, account, account2)
      ...
      await expect(tx)
        .to.emit(NameWrapperUpgraded, 'WrapARB2LD')
    ...
    it('Upgrades a .arb name if sender is authorised by the owner', async () => {
      ...
      const tx = await NameWrapper2.upgradeARB2LD(label, account2, account)
    ...
    it('Cannot upgrade a name if the upgradeContract has not been set.', async () => {
      ...
      await expect(
        NameWrapper.upgradeARB2LD(label, account, EMPTY_ADDRESS),
    ...
    it('Will pass fuses and expiry to the upgradedContract without any changes.', async () => {
      ...
      const tx = await NameWrapper.upgradeARB2LD(label, account, EMPTY_ADDRESS)
    ...
    it('Will burn the token, fuses and expiry of the name in the NameWrapper contract when upgraded.', async () => {
      ...
      await NameWrapper.upgradeARB2LD(label, account, EMPTY_ADDRESS)
    ...
    it('Will allow you to set the resolver on upgrade.', async () => {
      ...
      await NameWrapper.wrapARB2LD(label, account, 0, 0, DUMMY_ADDRESS)

      await NameWrapper.setUpgradeContract(NameWrapperUpgraded.address)

      await NameWrapper.upgradeARB2LD(label, account, DUMMY_ADDRESS)
    ...
    it('Does not allow anyone else to upgrade a name even if the owner has authorised the wrapper with the ENS registry.', async () => {
      ...
      await expect(
        NameWrapper2.upgradeARB2LD(label, EMPTY_ADDRESS, EMPTY_ADDRESS),
      ).to.be.revertedWith(`Unauthorised("${nameHash}", "${account2}")`)
    })
  })
  ...
  describe('setFuses()', () => {
    ...
    it('cannot burn PARENT_CANNOT_CONTROL', async () => {
      ...
      await expect(
        NameWrapper.setFuses(namehash('sub.abc.arb'), PARENT_CANNOT_CONTROL),
      ).to.be.revertedWith(
  ...
  describe('setChildFuses', () => {
    ...
    it('Allows .arb owners to set their fuses/expiry', async () => {
    ...
    it('Normalises expiry to the .arb registrar expiry', async () => {
      await registerSetupAndWrapName('fuses', account, 0, 0)
      ...
      await NameWrapper.setChildFuses(namehash('arb'), tokenId, 0, MAX_EXPIRY)
      // expiry in the .arb registrar
      const expectedExpiry = await BaseRegistrar.nameExpires(tokenId)
      ;[, , expiry] = await NameWrapper.getData(wrappedTokenId)
      expect(expiry).to.equal(expectedExpiry)
    })
    ...
    it('Normalises expiry to the old expiry', async () => {
      ...
      await NameWrapper.setChildFuses(namehash('arb'), tokenId, 0, 500)
      ;[, , expiry] = await NameWrapper.getData(wrappedTokenId)
      expect(expiry).to.equal(1000)
    })
  ...
  describe('registerAndWrapARB2LD', () => {
    ...
    it('should register and wrap names', async () => {
      await NameWrapper.registerAndWrapARB2LD(
        label,
        account,
        86400,
        EMPTY_ADDRESS,
    ...
    it('does not allow non controllers to register names', async () => {
      ...
      await expect(
        NameWrapper.registerAndWrapARB2LD(
          label,
          account,
          86400,
          EMPTY_ADDRESS,
    ...
    it('emits Wrap event', async () => {
      const tx = await NameWrapper.registerAndWrapARB2LD(
        label,
        account,
        86400,
        EMPTY_ADDRESS,
  ...
  describe('ERC1155 additional tests', () => {
    ...
    it('Approval on the Wrapper does not give permission to wrap the .arb name', async () => {
      ...
      await expect(
        NameWrapperH.wrapARB2LD(label, hacker, 0, 0, EMPTY_ADDRESS),
