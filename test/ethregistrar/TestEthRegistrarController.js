...
contract('ARBRegistrarController', function () {
  let ens
  let resolver
  ...
  it('should permit new registrations', async () => {
    const name = 'newname'
    const balanceBefore = await web3.arb.getBalance(controller.address)
    ...
    expect(
      (await web3.arb.getBalance(controller.address)) - balanceBefore,
  ...
  it('should permit new registrations with resolver and records', async () => {
    ...
    expect(await controller2.commitments(commitment)).to.equal(
      (await web3.arb.getBlock(tx.blockNumber)).timestamp,
    )

    await evm.advanceTime((await controller2.minCommitmentAge()).toNumber())
    var balanceBefore = await web3.arb.getBalance(controller.address)
  ...
  it('should not allow non token owners to renew a name with fuse and fuse expiration', async () => {
    ...
    ).to.be.revertedWith(`Unauthorised("${namehash('newname.arb')}")`)
  })
  ...
  it('should allow anyone to withdraw funds and transfer to the registrar owner', async () => {
    ...
    expect(parseInt(await web3.arb.getBalance(controller.address))).to.equal(0)
  })
